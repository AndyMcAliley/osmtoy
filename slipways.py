...
class Game:
    ...
    _top_div_height = 20
    _date_div = Div(text="Date: "+str(date),width=200,height=top_div_height)
    _money_div = Div(text="Money: ${:0,.2f}".format(money).replace('$-','-$'),width=200,height=top_div_height)
    _income_div = Div(text="Income: ${:0,.2f}".format(money).replace('$-','-$'),width=200,height=top_div_height)
    _message_div = Div(text=message,width=800,height=100)
    ...
    def node_select(self,attr,old,new):
        ...
        if len(node_cdf.selected.indices)>0:
            ...
            #_table_name.value = str(node_cdf.data['name'][new])
            _table_id.value = str(node_cdf.data['id'][selected_index])
            _table_name.value = str(node_cdf.data['name'][selected_index])
            _table_tag.value = str(node_cdf.data['tag'][selected_index])
            _table_area.value = ''
            _table_length.value = ''
        # else:
        #     _table_name.value = ''
    ...
    def update(self):
        _date_div.text = "Date: "+str(date)
        _money_div.text = "Money: ${:0,.2f}".format(money).replace('$-','-$')
        _message_div.text = message
